...
Ext.define("ESB.Rpc", {
	...
	requires: [ "ESB.window.MessageBox" ],
	singleton: true,
	autoAbort: false,
	...
	request: function(options) {
		...
		options = Ext.apply({
			timeout: ESB.HTTPREQUEST_TIMEOUT,
			relayErrors: true,
			showErrors: true
		}, options);
	...
	processResponse: function(options, success, response) {
		...
		if(!success) {
			...
			case ESB.E_SESSION_NOT_AUTHENTICATED:
				abort = true;
				reload = true;
				rpcResponse.message = _("Session not authenticated.");
				break;
			case ESB.E_SESSION_TIMEOUT:
				abort = true;
				reload = true;
		...
		if(!success) {
			...
			if(Ext.isDefined(options.relayErrors) && !options.relayErrors) {
				...
				if(options.showErrors)
					ESB.MessageBox.error(null, rpcResponse);
				return;
			}
		}
...
Ext.define("ESB.RpcObserver", {
	 ...
	 *   \li rpcData The RPC parameters. See ESB.Rpc.request.
	 *     \li service The name/class of the service to be executed.
	 *     \li method The method name to be executed.
	 *     \li params The parameters of the method to be executed as object.
	 *     \li options Optional RPC options. Defaults to NULL.
	...
	request: function(options) {
		...
		ESB.MessageBox.wait(options.title, options.msg);
		// Execute RPC.
