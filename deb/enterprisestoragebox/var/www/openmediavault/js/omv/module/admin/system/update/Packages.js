...
/**
 * @class ESB.module.admin.system.update.Packages
 * @derived ESB.workspace.grid.Panel
 */
Ext.define("ESB.module.admin.system.update.Packages", {
	...
	onUpgradeButton: function() {
		...
		Ext.Array.each(records, function(record) {
			...
			packages.push(name);
			// Is it a plugin?
			if(RegExp("^"+ESB.PRODUCT_PACKAGENAME+"(-\S+)?$", "i").test(
			  name)) {
				showMessageBox = true;
			}
		});
...
ESB.WorkspaceManager.registerPanel({
	...
	text: _("Updates"),
	position: 10,
	className: "ESB.module.admin.system.update.Packages"
});
