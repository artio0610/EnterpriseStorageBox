...
// require("js/esb/window/Upload.js")

/**
 * @class ESB.module.admin.system.plugin.Plugins
 * @derived ESB.workspace.grid.Panel
 */
Ext.define("ESB.module.admin.system.plugin.Plugins", {
	...
	columns: [{
		...
		renderer: function(value, metaData, record) {
			var tpl = Ext.create("Ext.XTemplate",
			  ...
			  '<tpl if="!Ext.isEmpty(values.extendeddescription)">',
				'{[ESB.util.Format.whitespace(values.extendeddescription)]}<br/><br/>',
			  '</tpl>',
			  '<tpl if="!Ext.isEmpty(values.size)">',
			    _("Size") + ': {[ESB.util.Format.binaryUnit(values.size)]}<br/>',
			  '</tpl>',
		...
		dataIndex: "size",
		stateId: "size",
		renderer: ESB.util.Format.binaryUnitRenderer(),
		width: 80,
		hidden: true
	},{
		text: _("Maintainer"),
	...
	onCheckButton: function() {
		var me = this;
		ESB.MessageBox.wait(null, _("Checking for new plugins ..."));
		// Execute RPC.
		ESB.Rpc.request({
			scope: me,
		...
		ESB.MessageBox.show({
			...
			fn: function(answer) {
				...
				var wnd = Ext.create("ESB.window.Execute", {
					...
					rpcMethod: "remove",
					rpcParams: { "packages": packages },
					rpcIgnoreErrors: [ ESB.E_ENGINED_CONNECT_SOCKET ],
					hideStartButton: true,
					hideStopButton: true,
					killCmdBeforeDestroy: false,
					listeners: {
...
ESB.WorkspaceManager.registerPanel({
	...
	text: _("Plugins"),
	position: 10,
	className: "ESB.module.admin.system.plugin.Plugins"
});
